...
export const useBookStore = defineStore('books', (): BookStoreProps => {
  ...
  let error = ref<boolean>(true);
  const books = ref<Book[]>([])
  const book = ref<Book | null>(null)

  async function getBook(id: number): Promise<void> {
    loading.value = true;
    error.value = false;
    try {
  ...
  async function purchaseBook(id: number): Promise<void> {
...
interface BookStoreProps {
  ...
  purchaseBook: (id: number) => Promise<void>
}
